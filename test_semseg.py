...
def main(args):
    ...
    with torch.no_grad():
        ...
        # Tahminleri kaydetmek için predictions klasörü oluştur
        pred_dir = Path('predictions')
        pred_dir.mkdir(exist_ok=True)
        # Test veri dosya isimlerini al
        test_files = [f for f in os.listdir(root) if f.endswith('.npy') and f.startswith('TestRay')]
        test_files = sorted([f for f in test_files if int(''.join(filter(str.isdigit, f))) >= 241 and int(''.join(filter(str.isdigit, f))) <= 300])
        file_idx = 0
        loader = torch.utils.data.DataLoader(TEST_DATASET, batch_size=BATCH_SIZE, shuffle=False)
        for i, (points, target) in tqdm(enumerate(loader), total=len(TEST_DATASET)//BATCH_SIZE+1):
